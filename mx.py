...
# 定时功能设置
target_time_tuple = (hour -1 , 59, 58,500)  # 小时, 分钟, 秒, 毫秒
...
PROXY_API_URL='http://http.tiqu.letecs.com/getip3?neek=321a408a&num=200&type=1&time=4&pro=0&city=0&yys=0&port=1&pack=0&ts=0&ys=0&cs=0&lb=1&sb=&pb=45&mr=2&regions=&cf=0&username=chukou01&spec=1'
# 抢券场次
    ...
    while len(all_proxies) < proxies_needed:
        ...
        if response.status_code == 200:
            if "请2秒后再试" in response.text:
                print("获取代理过于频繁，等待2秒后重试")
                time.sleep(2)
                continue
            if "白名单" in response.text:
                print("代理IP白名单验证失败，请联系管理员")
            else:
            ...
            async with aiohttp.ClientSession(connector=conn) as session:
                ...
                try:
                    ...
                    if response.status == 405 or "您的访问被阻断" in response_text:
                        ...
                        print(f"账号 {remark} {post_time} 服务器返回了无效的JSON响应: {response_text}，当前代理IP: {proxy_info}")
